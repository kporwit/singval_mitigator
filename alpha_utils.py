...
def create_alpha(singval_tab, eigenval_tab, verbosity):
    ...
    if verbosity > 1:
        print('-2*10^-16*n*max(singval_tab) =',-2e-16*n*max(singval_tab))
    ...
    if temp < -2e-16*n*max(singval_tab):
        if verbosity > 0:
            ...
            print('porownanie temp: ', -2e-16*n*max(singval_tab))
        #sys.exit()
        b = np.ones(3)
        return b
    for i in range(0, n - 1):
        ...
        if correction > 0:
            if verbosity > 0:
                print('j+1')
                print([a[j+1, j+1], eigenval_tab[i]])
            b = np.ones(3)
            return b
